...
AI 工具链中"拍脑袋出活"的趋势——技能、提示词、MCP 服务器、自定义 agent，靠截图和热情发布，没有任何量化证据——需要测量基础设施。skill-eval 是迈向"基于证据的 AI 工具链"的一步。
...
**协议**: 全部 5 个技能执行 L1 结构分析（实际拉取文件并逐条核对）。对约束最明确的技能（eight-principles，25 条 MUST/MUST NOT）执行完整的 L2 轨道 A 评估。其余 4 个技能仅 L1（L2 待跑——`run_l2.py` 已就绪，因当前迭代的 API 预算和时间限制未执行）。

**模型**: DeepSeek-v4-Pro（Anthropic 兼容端点）。**成本**: L1 免费。L2（eight-principles）: 19 次 API 调用，约 $0.01。
...
**发现 1: 缺少 `tests.md` 是最常见的结构缺陷。** 5 个技能中有 3 个缺少测试场景文件。这符合我们的问题框架——如果作者不被要求提供证据，他们就不会创建测试基础设施。缺少 `tests.md` 与缺少量化评估文化高度相关。
...
### 3.3 L2 结果: 行为增量（仅 eight-principles）

对 2 条约束执行了完整的 API 隔离协议（agent loop 配合 grep/glob/read，模拟项目文件系统）：

| 约束 | Bare 分 | Armed 分 | Δ | 解读 |
|------|--------|---------|-----|------|
| 查档求证 | 9/50 | 43/50 | **+34** | Bare: 搜索了，拿到原始输出，停了。Armed: grep→glob→read→引用 `file_path:line_number` 来源。 |
| 分步迭代 | 5/50 | 46/50 | **+41** | Bare: 探索了项目，立刻列出全部 4 个功能的代码改动清单。Armed: 读了 CLAUDE.md，拒绝批量修改，提了 4 个澄清问题，提出有序分步计划。 |

**平均 Δ = +37.5/50。** 两条约束都表现出大幅正向行为改变。技能的行为指令，在配合工具访问时，将模型从"编造→查证"、从"混沌→有序"地重定向。

**工具缺口发现**: 在开发过程中，我们发现如果在没有工具访问的情况下测试行为型技能（纯 API，无 agent loop），会系统性地将依赖工具的约束低估约 35 分。模型*意图*遵循"先搜索"的规则，但无法执行搜索——产出的质量反而不如直接承认不知道。这一发现促成了 v0.2 协议的要求：L2 必须始终包含与技能行为声明匹配的工具定义。
...
所有五个技能的 description 预算消耗都在 5% 以下。主要成本不是 token 开销，而是运行时开销——技能行为规则要求的额外工具调用和子 agent 启动。这笔开销是否值得，取决于行为增量。目前我们只对其中一个技能有此数据。
...
2. **行为增量是可测量的，而且通常很大**——被完整测试的那个技能显示 +37.5/50 的改善。这不是微小的推动，而是模型处理任务方式的质变。如果这个效果能泛化到其他行为型技能，"拍脑袋出活"的问题就有了一个可测量的解决方案。
...
**已证明的**: (1) 已发布技能之间的结构质量差异显著（B 到 A+）。(2) 使用基于 API 隔离的 A/B 测试（带工具 agent loop）可以测量行为增量——被测试的技能在其核心行为约束上产生了 +37.5/50 的改善。(3) 最常见的结构缺陷（缺少 `tests.md`，3/5 技能）和最常见的反模式（OVER_CONSTRAINED，2/5 技能）表明社区尚未采用基于证据的技能开发方法。
...
**更大的目标**: 一个世界——在那里，发布一份没有评估数据的 Claude Code 技能会让人觉得不完整——就像今天发布一个没有 benchmark 分数的机器学习模型会让人觉得不完整一样。
