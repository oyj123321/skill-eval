...
**尚待完成**: (1) Claude 全层级（Sonnet/Opus/Haiku）多模型测试，绘制 skill 价值随模型能力变化的完整曲线。(2) 实现轨道 C（格式型）和轨道 D（工具型），完成 5 轨道框架。(3) 蒙特卡洛重复以获得统计置信区间。(4) 更大规模、随机采样的技能语料——包含低质量技能以验证评估器的区分能力。
